...
if [ "${PUSH_REWARD_ABLATION_MODE}" = "baseline" ]; then
  PUSH_SUFFIX="${PUSH_SUFFIX:-g1-push-${PROFILE}-${PUSH_REWARD_MODE}-seed${SEED}-${PIPELINE_ID}}"
else
  PUSH_SUFFIX="${PUSH_SUFFIX:-g1-push-${PROFILE}-${PUSH_REWARD_ABLATION_MODE}-seed${SEED}-${PIPELINE_ID}}"
fi
...
echo "[pipeline] push:  envs=${PUSH_NUM_ENVS} eval_envs=${PUSH_NUM_EVAL_ENVS} timesteps=${PUSH_NUM_TIMESTEPS} suffix=${PUSH_SUFFIX} reward_mode=${PUSH_REWARD_MODE} reward_ablation_mode=${PUSH_REWARD_ABLATION_MODE}"
...
if [ "$(stage_rank "${START_STAGE}")" -le 3 ]; then
  ...
  run_env_script "stage=push" "${ROOT_DIR}/scripts/train_g1_push_recovery.sh" \
    ...
    "PUSH_REWARD_ABLATION_MODE=${PUSH_REWARD_ABLATION_MODE}" \
