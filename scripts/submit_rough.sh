...
if [ "${USE_CUDA}" = "1" ]; then
  ...
  SBATCH_SCRIPT="${ROOT_DIR}/slurm/g1_rough_cpu.sbatch"
...
echo "[submit-rough] partition=${PARTITION} gpus=${GPUS} cpus=${CPUS_PER_TASK} mem=${MEM_PRINT} time=${TIME_LIMIT} use_cuda=${USE_CUDA} jax_extra=${JAX_CUDA_EXTRA:-cuda12}"
